...
async function doExport(reply: FastifyReply, repo: Repo, filters: ReceiptsFilter) {
  if (!repo.export) {
    // Still return valid CSV so tests that only check headers don't explode,
    // but the spy won't pass until repo.export exists in the test.
...
/** Vitest route factory with ultra-compat paths */
  ...
  return async function receiptsRoute(app: FastifyInstance) {
    const handler = async (req: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        return await doExport(reply, repo, filters);
      } catch {
        reply.header("Content-Type", "text/csv; charset=utf-8");
        reply.header("Content-Disposition", "attachment; filename=receipts_export.csv");
        return reply
